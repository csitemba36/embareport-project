...
$res = $mysqli->query("
    SELECT initial_maserp_kd_gudang, name
    FROM yo_store
");
while ($r = $res->fetch_assoc()) {
    $positionMap[$r['initial_maserp_kd_gudang']] = $r['name'];
}
...
$where[] = "g.KodeGudang IN ($validGudangSql)";
...
WITH base AS (
    SELECT
        ...
        MAX(g.KodeGudang) AS KodeGudang,
...
while ($row = odbc_fetch_array($result)) {
    ...
    $data[] = [
        ...
        "BRAND"        => $row['NamaMerk'],
        "InitialStore" => $storeMap[$row['KodeGudang']] ?? null,

        "Position"     => $positionMap[$row['KodeGudang']] ?? null,
