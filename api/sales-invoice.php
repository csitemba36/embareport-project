<?php
require_once('../config/connect_db.php'); // koneksi SQL Server ODBC
...
while ($row = odbc_fetch_array($result)) {
    ...
    $harga_net   = ($harga_jual * $qty) - ($qty * $diskon);
    $harga_net_formatted = number_format($harga_net, 0, ',', '.');

    // Ambil data MySQL berdasarkan kode_gudang
    $kodeGudang = $mysqli->real_escape_string($row['KodeGudang']);
    $resMy = $mysqli->query("SELECT * FROM m_wilayah WHERE kode_gudang = '$kodeGudang' LIMIT 1");
    $wilayahData = $resMy->fetch_assoc() ?? [
        "wilayah" => "", "area" => "", "kota" => "", 
