...
<script>
    $(document).ready(function() {
        ...
        //KALAU SERVESRSIDE TINGGAL AKTIFKAN
        /*let table = $('#stock-table').DataTable({
            ...
            ajax: {
                ...
                url: 'api/get_inventory_clientside.php',
                type: 'POST',
                data: function (d) {
                    // Ambil semua nilai filter
                    let merk   = $('#filter-merk').val();
                    let range  = $('#filter-range').val();
                    let style  = $('#filter-style').val();
                    let bahan  = $('#filter-material').val();
                    let warna  = $('#filter-color').val();
                    let size   = $('#filter-size').val();
                    let gudang = $('#filter-gudang').val();
                    ...
                    d.warna       = warna;
                    d.size        = size;
                    d.kode_gudang = gudang;

                    // Debug di console
                    console.log("Filter saat ini:", { merk, range, style, bahan, warna, size, gudang });
                },
        ...
        // Event untuk batalkan request kalau semua filter kosong
        $('#stock-table').on('preXhr.dt', function (e, settings, data) {
            ...
            if (semuaKosong) {
                e.preventDefault(); // Batalkan request ke server
        ...
        let table = $('#stock-table').DataTable({ 
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.partnumber  = $('#filter-partnumber').val(); // nilai dari select biasa
                },
                dataSrc: function (json) {
            ...
            columns: [
                ...
                { 
                    data: 'HargaJual',
                    className: 'text-end'   // 👈 Bootstrap 5 (rata kanan)
                },
                { data: 'wilayah' },
				{ data: 'area' },
				{ data: 'kota' },
