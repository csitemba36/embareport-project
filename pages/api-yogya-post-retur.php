...
$(document).ready(function() {
    ...
    var table = $('#tspdo1Tabledua').DataTable({
        ...
        ajax:   {
                    url: "api/db-processing-transfer-retur.php",
                    data: function (d) {
...
$(document).on('click', '.btn-post-transaction', function () {
    ...
    Swal.fire({
        ...
        let details = items.map(item => ({
            supplier_barcode: item.KodeItem,
            qty_return: parseInt(item.QtyJual) || 0
        ...
        // ===== AJAX POST =====
        $.ajax({
            url: 'api/yogya-api-service/db-post-retur.php',
            type: 'POST',
